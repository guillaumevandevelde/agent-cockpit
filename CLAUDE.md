...
- **Twee aparte SQLite-stores** — verwar ze niet bij een board-dive: de **registry-DB** staat op `backend/claude_registry.db` (MCP servers, commands, permissions, plugin-state); het **kanban-bord** zit in `~/.claude-registry/kanban.db` (`kanban_cards`, `kanban_columns`, `kanban_meta`, activity-feed — `backend/app/config.py:21-29` + `:65-69`, "Separate store for the kanban board domain, portable, sync-able, one-per-machine"). Een query tegen de verkeerde geeft `sqlite3.OperationalError: no such table: kanban_meta` (of omgekeerd) en kost een `config.py`-grep.
...
- **Een stub-listener starten via `&` / `nohup` / `run_in_background: true` wordt geweigerd in een gedispatchte sessie.** Voor een stub HTTP-server (of een ander proces dat een listening socket opent en langer leeft dan het trigger-commando) krijg je een deny — niet van een shell-rule, maar van de dispatch-permission-laag — ongeacht of je `nohup python stub.py & ; echo $! > pidfile`, de Bash-tool met `run_in_background: true` op hetzelfde commando, of een inline `python stub.py & ; SERVER_PID=$! ; ... ; kill "$SERVER_PID"` in één Bash-call gebruikt. Een plain `sleep 3 &` lukt wél, dus de weigering is specifiek voor processen die op een open socket blijven hangen, niet voor `&` an sich. Symptomen uit kaart `fca8abf7…`: drie opeenvolgende denies op een stub-Anthropic-compatible HTTP-server (`http.server.HTTPServer`) vóór het patroon werd gevonden — elke poging "ik heb de PID toch al expliciet vast, mag het nu wel?" werd alsnog geweigerd. **Workaround — background-thread in één foreground-process:** draai de listener in een `threading.Thread(target=srv.serve_forever, daemon=True)` binnen één Python-script dat óók de te testen subprocess aanstuurt, zodat het script zelf één blocking foreground Bash-call blijft (geen `&`, geen `run_in_background`) en de thread meeleeft zolang het script loopt. De subprocess kan dezelfde proces-context delen (request naar de stub, response afvangen in dezelfde Python-interpret), en afsluiten gaat via `srv.shutdown()` vanuit dezelfde foreground-thread aan het eind van het script. Van toepassing op alle sessies die een "echte spawn"-bewijs moeten leveren voor een CLI-adapter (de "bewijs met een echte spawn"-conventie in kaarten zoals `2f3776dd…`), of breder: élke integratie-test die een lokale mock/stub-netwerkservice nodig heeft om end-to-end te bewijzen dat een pad werkt.
...
- **Resume-mode heeft twee geldige namen: een verse tmux-/claim-id en de bestaande worktree-/transcript-id.** `_run_card` mint bij élke dispatch een nieuwe sessienaam (`claimed_by=agent:<nieuw>`, `dispatch_started_at` springt vooruit), maar `make_resume_transport` start `claude --resume` in de bestaande `resume_project_folder`. Een volledige verse dispatch-prompt kan daardoor in dezelfde conversatie/worktree `<oud>` verschijnen terwijl het bord `<nieuw>` toont — dat is op zichzelf geen reap en geen tweede werkende agent. Extra val: `dispatch_project_folder` wordt momenteel opgebouwd uit `<nieuw>` in plaats van de door resume opgeloste cwd en kan dus naar een map wijzen die nooit bestond. Gebruik voor de echte lineage de spawn-regel `Spawned ... in <dir> (mode=resume)`, `resume_project_folder` en de aanwezige `~/.claude/projects/<folder>`; bewijs een botsing pas met een extra claim/release-cyclus plus twee levende tmux/processen. Geobserveerd op kaart `21047ce2…`: tmux/claim `k-resolve-imped-1198` hervatte worktree/transcript `k-resolve-imped-e371` zonder reaper-event.
...
- **Ongequote zsh-metatekens in commando-argumenten zijn een silent no-op.** De dispatch-shell is zsh (zie omgevingsblok). Twee klassen bijten hier: (1) **glob-tekens** `?`, `*` en `[` — die globben standaard tegen de cwd, en bij geen match geeft de shell een onschuldige `no matches found`-fout waardoor het commando **nooit draait** (`grep -rn "FOO" backend/app --include=*.py` → `(eval):1: no matches found: --include=*.py`); (2) **EQUALS-expansie** — een woord dat met `=` begint wordt vervangen door het pad van het gelijknamige commando, dus `echo ===` faalt met `(eval):1: == not found`. In alle gevallen leest de foutmelding niet als een quoting-fout maar als "leeg resultaat / commando stuk", wat een of meer retries kost om te ontdekken. Fix: quote de hele waarde met enkele of dubbele quotes (`grep --include="*.py"`, `echo "==="`). Twee veel-voorkomende oppervlakken in deze codebase met hetzelfde faalpatroon:
