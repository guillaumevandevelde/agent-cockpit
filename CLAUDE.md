...
- **`pkill -f`/`pgrep -f` in gedispatchte sessies riskeert een concurrente agent-sessie op deze box.** Sinds Claude Code 2.1.214 weigert `pkill` de eigen CLI (geverifieerd op lokale CLI 2.1.221); het *blijvende* risico zit in andermans sessie — de dispatcher zet de hele prompt in de cmdline (`backend/app/services/agentic_cli/claude_code.py:82-83`).
