...
- **Refactor-frontend-orphan-sweep bij backend-Pydantic-rename of -veldverwijdering.** Een `backend/app/models/*_schemas.py`-veld dat verdwijnt, laat een TS-reference in `frontend/src/` achter waar `npm run build` weken later op faalt — backend pytest + OpenAPI-snapshot zien het niet. Draai vóór de commit `grep -rn "<verwijderd_of_hernoemd veld>" frontend/src/` per aangeraakt veld; geen treffers = schoon. Aanvulling op `scripts/check-schema-rename-coverage.sh` (dekt alleen backend → backend). Volledige recept + voorbeeld-orphan: [`docs/cockpit/refactor-frontend-orphan-sweep.md`](./docs/cockpit/refactor-frontend-orphan-sweep.md) (kaart `39778adba3e74d2b8fd6f786344f4a8d`).
