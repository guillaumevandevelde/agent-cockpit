...
- **`gh` zonder expliciete repo leest de fork-upstream (`adrirubio/claude-deck`) — CI-, run- en PR-data van de verkeerde repo, zonder foutmelding.** Deze repo heeft twee remotes: `origin` = `guillaumevandevelde/agent-cockpit` (waar de cockpit zelf landt) en `upstream` = `adrirubio/claude-deck` (de fork-bron, zie fork-header boven aan dit bestand). `gh` resolvet zonder repo-argument standaard naar `upstream`, dus `gh repo view` → `{"nameWithOwner":"adrirubio/claude-deck"}`, `gh run list` toont andermans runs, `gh pr list` andermans PR's. Fix: geef het repo expliciet mee (`gh <cmd> guillaumevandevelde/agent-cockpit …` of `-R guillaumevandevelde/agent-cockpit`), of zet eenmalig `gh repo set-default guillaumevandevelde/agent-cockpit` (device-state — overleeft geen verse checkout, deze doc is het duurzame alternatief).
