 ...
 *   - Two-column grid: question on the left (scrolls within its column at
 *     `lg:max-h-[70vh]`), action surface on the right. The grid is no longer
 *     capped by a modal — at viewport sizes below `lg` it falls back to a
 *     single column so laptop users still see both halves.
...
function ResolveImpediment({
  ...
  return (
    <UiCard>
      ...
      <CardContent>
        {/* Two-column on lg+: the long question scrolls on the left, the action
            surface anchors on the right. Mirrors the previous modal split but
            with a `lg:max-h-[70vh]` cap (vs 55vh) — the page has a tall
            viewport, so the cap can be more generous without pushing the
            Resolve button out of reach. Below `lg` the grid collapses to a
            single column. */}
        <div
          className="grid grid-cols-1 gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(360px,420px)]"
          data-testid="impediment-split-layout"
        >
          <div
            className="min-h-0 lg:max-h-[70vh] lg:overflow-y-auto lg:overscroll-contain"
