...
Agent Cockpit is a self-hosted web application for visualizing and managing local AI coding agents. It started with [Claude Code](https://docs.anthropic.com/en/docs/claude-code) configuration and now includes stable Codex CLI support for tmux sessions, safe TOML settings, feature flags, diagnostics, MCP/plugin inventory, and redacted config exports.

## Why Agent Cockpit?

Claude Code stores configuration across multiple JSON files and directories (`~/.claude.json`, `~/.claude/settings.json`, `.claude/settings.json`, `.mcp.json`, and more). Managing these files manually is tedious and error-prone. Agent Cockpit gives you a visual dashboard to:
...
Agent Cockpit covers Claude Code configuration and the shared local-agent operations layer:
...
- [Installation](/guide/installation) — get Agent Cockpit running locally
