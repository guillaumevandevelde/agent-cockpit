...
| 4 | **Autonome opbouw.** Vanaf hier neemt de bestaande multi-agent kanban-flow het over: de eerste Backlog-kaart wordt door de analyst-fase in kind-kaarten gesplitst en door executors gebouwd — dezelfde machine als het meta-project zelf gebruikt. | Dispatcher | `backend/app/kanban/dispatch.py`, `multi-agent-kanban.md` |
