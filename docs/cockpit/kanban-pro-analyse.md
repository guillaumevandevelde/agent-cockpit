...
✅ **Geïmplementeerd (kaart 985311943f3044bab9b5b914a7629c7f)** — `SECURITY.md` benoemt nu de vier gevraagde onderdelen: het vertrouwensmodel (kaarttekst is een prompt), bewuste ontwerpkeuzes, aanwezige én ontbrekende controls, en in-/out-of-scope voor rapporteurs. De beschrijving maakt ook de huidige permissieve rand expliciet: een geregistreerd project zonder gematerialiseerde `ProjectSecurityProfile`-rij valt terug op host-worktree + skip-permissions; de `product-staging`/Sandcastle-modeldefaults gelden pas nadat REST/UI de profielrij heeft aangemaakt. Geen codewijziging in deze kaart.
