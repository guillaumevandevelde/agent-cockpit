...
> **Afgeleid uit de frontmatter — niet met de hand bewerken.** Regenereer met `scripts/generate-doc-index.py`; `scripts/generate-doc-index.py --check --strict` bewaakt de drift. Dekt **alle 91 docs** (elke `docs/cockpit/*.md`), gegroepeerd op `type` met een `status`-badge.
