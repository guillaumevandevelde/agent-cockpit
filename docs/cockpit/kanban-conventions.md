...
| `**Dependency removed:** ` | (geen) | `service.strip_dangling_deps_on_delete`, vóór een card-delete (los én via "Clear Done") | Puur documentair: legt vast dat een `depends_on`-id is gestript omdat de bron-kaart verdween. Bewust géén consumer-prefix. |
| `**Parent removed:** ` | (geen) | `service.orphan_children_on_delete`, vóór een card-delete (los én via "Clear Done") | Zusje van de vorige, voor `parent_card_id`. Legt vast dat de parent-link is gewist omdat de parent verdween — inclusief het plan dat die parent nooit meer gaat leveren. |
...
| Wat moet er in de acceptance criteria van een removal-/deprecatiekaart (route, tabel, store)? | Een expliciete **in-repo** caller-sweep over frontend én overige backend — niet alleen "geen externe tooling gebruikt dit". Volledige guard: `.claude/agents/analyst.md` → *Guards tegen Backlog-spam*, gespiegeld in `backend/app/kanban/analyst_prompt.py`. |
