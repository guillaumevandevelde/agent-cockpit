...
def _build_ship_instructions(ship_mode: str, project_path: str | None = None) -> str:
    ...
    if ship_mode == "direct":
        shipping = (
            ...
            "through (kanban card efb8187b…).\n"
            "     CONFLICTED=$(git -C \"$WT\" diff --name-only --diff-filter=U "
            "| LC_ALL=C sort -u)\n"
            "     EXPECTED=$(printf 'docs/cockpit/README.md\\ndocs/cockpit/"
            "llms.txt\\n' | LC_ALL=C sort -u)\n"
            "     if [ \"$CONFLICTED\" != \"$EXPECTED\" ]; then\n"
            "       echo \"ERROR: merge conflict in non-generated files — "
            "falling back to report_impediment.\" >&2\n"
            "       printf '  conflicted: %s\\n' $CONFLICTED >&2\n"
            "       echo \"Conflicted worktree left at $WT for inspection "
            "(not removed).\" >&2\n"
            "       exit 1\n"
            "     fi\n"
            "     # Carve-out: conflict set is exactly the two generated "
            "doc-index files. `--theirs` clears the merge markers; the next "
            "regenerate step overwrites both files anyway, so `--theirs` vs "
            "`--ours` is moot in practice. The script MUST be invoked "
            ...
            "--diff-filter=U``. When that set is **exactly** "
            "``docs/cockpit/README.md`` and ``docs/cockpit/llms.txt``, the script "
            "runs the carve-out automatically and the merge completes inline "
            "— no human intervention. Both files are fully regenerated by "
            "``scripts/generate-doc-index.py`` from the frontmatter of "
            "``docs/cockpit/*.md``; concurrent docs-sessions each regenerate "
            "from their own frontmatter snapshot, the merged frontmatter is "
            "the union, and the regenerate inside ``$WT`` reconciles from that "
            "union. **Why the conflict must remain visible** "
