...
def _build_ship_instructions(ship_mode: str, project_path: str | None = None) -> str:
    ...
    if ship_mode == "direct":
        ...
        shipping = (
            ...
            "   gh pr ready\n"
            "   # The PR merges the REMOTE tip, not your working copy. Any\n"
            "   # commit made after the push above is invisible to the merge\n"
            "   # and is silently dropped — nothing errors, the PR goes green\n"
            "   # without it. Re-assert the tips match before queueing.\n"
            "   git fetch origin -q\n"
            "   BRANCH=$(git rev-parse --abbrev-ref HEAD)\n"
            "   if [ \"$(git rev-parse HEAD)\" != "
            "\"$(git rev-parse \"origin/$BRANCH\" 2>/dev/null || echo missing)\" ]; then\n"
            "     git push origin HEAD || { echo 'push failed; do NOT merge'; exit 1; }\n"
            "   fi\n"
            "   gh pr merge --auto --squash\n"
            "   ```\n"
            "   Capture the PR URL from ``gh pr create`` output.\n"
            "   **Commit nothing after this point** — ``--auto`` merges as soon "
            "as checks pass, so a later commit races the merge and loses. After "
            "the merge, confirm your work actually landed (\"PR merged\" and \"my "
            "commits shipped\" are different statements): ``git fetch origin -q`` "
            "then grep your final commit's subject in ``git log origin/master``. "
            "If it is absent, push and open a follow-up PR instead of reporting "
            "Done.\n"
