...
def _build_mcp_fallback_instructions() -> str:
    ...
    **not optional**, even when `depends_on_graph={}` (an independent child
    ...
    return (
        ...
        "- `POST /cards/{id}/move` — body `{\"column\": \"…\"}` (for Done, "
        "follow with a `comment` carrying your summary — the REST move has no "
        "`summary` field. `column=\"Impediment\"` is rejected with 422; the "
        "Impediment route is `report_impediment`'s only, see kaart b8e3ac8b… "
        "decision A)\n"
...
def _build_ship_instructions(ship_mode: str, project_path: str | None = None) -> str:
    ...
    tests = (
        ...
        "necessary but not sufficient (card 9b7c2a98…). Force Node to resolve "
        "eslint's full module graph so a missing transitive dep like ``acorn`` "
    ...
    if ship_mode == "direct":
        shipping = (
            ...
            "branches piled up over 6 weeks). Guard the delete on the "
            "remote ref actually existing: a direct-mode branch that never "
            "made it to `origin` yields two `error:` lines from `git push "
            "--delete` that read like a failed ship, even though the "
            "push-to-master already succeeded — and that spurious output "
            "reliably trips \"the ship failed\" reading on the tail of the "
            "log (kanban card 552036fa…). Fail-open — an already-deleted "
