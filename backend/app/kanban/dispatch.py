...
def _build_ship_instructions(ship_mode: str, project_path: str | None = None) -> str:
    ...
    if ship_mode == "direct":
        shipping = (
            ...
            "merge once.\n\n"
            "   A conflict that is **exclusively** in the generated documentation "
            "artifacts ``docs/cockpit/README.md`` and ``docs/cockpit/llms.txt`` "
            "is deterministic. **Verify the conflict set before touching "
            "anything** — a handwritten conflict MUST go to a human, never to "
            "this carve-out:\n\n"
            "   ```bash\n"
            "   # Enumerate the unmerged paths and verify the set is EXACTLY "
            "the two generated doc-index files. Anything else (a hand-written "
            "conflict) aborts the carve-out and falls through to "
            "``report_impediment`` below. The condition must be "
            "machine-checkable, not a judgement call — see kanban card "
            "``efb8187b…`` for the failure mode that motivated this.\n"
            "   CONFLICTED=$(git -C \"$WT\" diff --name-only --diff-filter=U "
            "| LC_ALL=C sort -u)\n"
            "   EXPECTED=$(printf 'docs/cockpit/README.md\\ndocs/cockpit/"
            "llms.txt\\n' | LC_ALL=C sort -u)\n"
            "   if [ \"$CONFLICTED\" != \"$EXPECTED\" ]; then\n"
            "     echo \"ERROR: conflict set is not exactly the generated "
            "doc-index pair — falling back to report_impediment.\" >&2\n"
            "     printf '  conflicted: %s\\n' $CONFLICTED >&2\n"
            "     exit 1\n"
            "   fi\n"
            "   ```\n\n"
            "   If the check passes, the conflict is deterministic. Resolve "
            "each generated file with ``--theirs`` (the branch being merged just "
            "regenerated these from its own frontmatter on its own ship; "
            "``generate-doc-index.py`` overwrites the placeholder on the next "
            "step anyway), then **regenerate from the surviving "
            "``docs/cockpit/*.md`` frontmatter inside ``$WT``** — "
            "``scripts/generate-doc-index.py:78`` derives its repo-root from "
            "``Path(__file__).resolve().parent.parent``, so the script MUST be "
            "invoked through the worktree path. A bare ``cd $WT`` is NOT "
            "enough: the script resolves its parent relative to its own "
            "location, not to the shell's cwd, so ``./scripts/generate-doc-index.py`` "
            "regenerates the wrong tree, the strict check passes against the "
            "wrong tree, and the conflicted worktree ships unmerged markers. "
            "Then complete the merge, push, and clean up:\n\n"
            "   ```bash\n"
            ...
            "   git worktree remove --force \"$WT\"\n"
            "   ```\n\n"
            "   The strict check must pass before pushing. "
            ...
            "The carve-out's \"conflict → regenerate\" loop is the right "
            "pattern: the conflict acts as a freshness alarm, and the "
            "regenerate step reconciles from the merged frontmatter.\n\n"
            "   If **any handwritten file is also in the conflict set**, the "
            "carve-out does not apply: follow the existing rule, "
