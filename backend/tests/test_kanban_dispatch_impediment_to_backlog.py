...
# ---------------------------------------------------------------------------
# Strand-bug regression tests — kaart acc8f578…
#
# When a card was claimed by an agent while it sat on Impediment
# (`apply_operation(op_type="move", …, payload={"column": "Impediment"})`
# does NOT trigger the operations.py:327-333 agent→fixed guard, because
# both source and destination are fixed columns), the prior
# `dispatch_impediment_card` parked it on Backlog with the claim intact.
# `_next_card` filters `not c.claimed_by`, so the card became permanently
# invisible to auto-dispatch. The fix clears `agent:*` claims on resolve,
# mirroring the operations.py:327-333 guard; human `me@ui` reservations are
# preserved because a deliberate human hold survives a column move.
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_dispatch_impediment_card_clears_agent_claim_on_resolve():
    """AC #1 (strand bug fix): a card claimed by `agent:*` while on Impediment
    must end up dispatchable on Backlog after resolve — i.e. its `claimed_by`
    is cleared, so the next `_next_card` filter picks it up. Before the fix,
    the claim survived the Backlog move and the card was stranded."""
    async with KanbanSessionLocal() as s:
        cid = await _make_impediment_card(s)
        # Claim the card while it sits on Impediment (the gap the operations.py
        # agent→fixed guard never covers — both source and destination columns
        # are fixed). Mirrors the real reproduction in the card description
        # (`claim_card` against an Impediment card via MCP).
        await apply_operation(
            s, op_type="claim", entity_type="card", project_key=PK,
            entity_id=cid, payload={"claimed_by": "agent:k-zombie-1234"},
        )
        await s.commit()

    async with KanbanSessionLocal() as s:
        await dispatch.dispatch_impediment_card(
            s, card_id=cid, project_path="/tmp",
            target_agent="engineer",
            impediment_question="Which library should we use?",
            impediment_answer="Use library B.",
    ...
    async with KanbanSessionLocal() as s:
        card = await get_card(s, cid)
    assert card.column == "Backlog", (
        f"expected Backlog after resolve, got {card.column!r}"
    )
    assert card.claimed_by is None, (
        f"agent: claim must be cleared on resolve-impediment — otherwise "
        f"_next_card's `not c.claimed_by` filter strands the card forever. "
        f"Got claimed_by={card.claimed_by!r}"
    )
    assert card.claimed_at is None


@pytest.mark.asyncio
async def test_dispatch_impediment_card_preserves_human_me_ui_reservation():
    """AC #2 (strand bug fix): a card reserved by a human `me@ui` while on
    Impediment must KEEP its claim after resolve. A deliberate human hold is
    an intentional reservation — silently stripping it on resolve would
    surprise the operator who clicked Reserve, then Discover later that the
    card was already in flight. This matches the operations.py:327-333 guard's
    carve-out for non-agent claimants."""
    async with KanbanSessionLocal() as s:
        cid = await _make_impediment_card(s)
        await apply_operation(
            s, op_type="claim", entity_type="card", project_key=PK,
    ...
    assert card.claimed_by == "me@ui", (
        f"human me@ui reservation must survive resolve-impediment — the "
        f"operator deliberately held the card. Got claimed_by={card.claimed_by!r}"
    )


@pytest.mark.asyncio
async def test_dispatch_impediment_card_unblocked_card_dispatches_after_resolve():
    """End-to-end AC: an agent-claimed-then-resolved card actually gets picked
    up by the next auto-tick. Combines the strand fix with the existing
    `dispatch_project` flow — a unit test on `claimed_by` alone would miss a
    regression where the fix touches a field the dispatcher doesn't filter on.
    """
    async with KanbanSessionLocal() as s:
        cid = await _make_impediment_card(s)
        await apply_operation(
    ...
    # Resolve — claim should be cleared.
    async with KanbanSessionLocal() as s:
        await dispatch.dispatch_impediment_card(
            s, card_id=cid, project_path="/tmp",
            target_agent="engineer",
    ...
    # Next auto-tick must pick it up: the prior code's strand bug would have
    # silently skipped this card because `claimed_by` was still set.
    transport = RecordingTransport()
    async with KanbanSessionLocal() as s:
        await dispatch.dispatch_project(
            s, project_key=PK, project_path="/p", transport=transport,
    ...
    assert len(transport.calls) == 1, (
        f"auto-tick must dispatch the resolved card; got {len(transport.calls)} "
        f"spawn(s) — a 0-spawn result here is the strand bug resurfacing."
    )
    assert transport.calls[0]["card_id"] == cid