...
def test_discover_teams_groups_by_cwd():
    """Sessions sharing cwd + provider are grouped into an auto-detected team."""
    from app.services.agent_bridge.teams import discover_teams
    ...
    teams = discover_teams(sessions)
    assert len(teams) == 1
    team = teams[0]
    assert team["is_auto_detected"] is True
    assert team["cwd"] == "/repo/a"
    assert len(team["members"]) == 2
    assert team["members"][0]["session_name"] == "lead-aaaa"
    assert team["members"][1]["session_name"] == "member-bbbb"


def test_discover_teams_skips_single_session():
...
def test_discover_teams_separates_by_provider():
...
def test_get_ungrouped_sessions():
    """Sessions in teams are excluded from the ungrouped list."""
    from app.services.agent_bridge.teams import discover_teams, get_ungrouped_sessions
    ...
    teams = discover_teams(sessions)
    ungrouped = get_ungrouped_sessions(sessions, teams)

    assert len(ungrouped) == 1
    assert ungrouped[0]["session_name"] == "solo"


def test_discover_teams_with_manual_teams():
    ...
    manual_teams = [
        {
            ...
            "lead_session_name": "manual-lead",
            "members": [
                {"session_name": "manual-lead", "pane_id": None, "tmux_target": "manual-lead:0.0"},
                {"session_name": "manual-follower", "pane_id": None, "tmux_target": "manual-follower:0.0"},
            ],
        }
    ]

    teams = discover_teams(sessions, manual_teams)
    assert len(teams) == 1
    assert teams[0]["is_auto_detected"] is False
    assert teams[0]["name"] == "My Team"
    assert teams[0]["team_id"] == "manual-1"
