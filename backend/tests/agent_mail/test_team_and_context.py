...
@pytest.mark.asyncio
async def test_list_team_reports_status(tmp_path):
...
@pytest.mark.asyncio
async def test_build_session_start_context_includes_identity_and_roster(tmp_path):
    with patch("app.services.agent_mail_service.discover_agent_sessions", return_value=[]):
        async with AsyncSessionLocal() as s:
            (tmp_path / "a").mkdir()
            (tmp_path / "b").mkdir()
            _, _ = await agent_mail_service.register_session(
