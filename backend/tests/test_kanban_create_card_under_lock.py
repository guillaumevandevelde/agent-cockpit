...
async def test_create_card_returns_503_when_the_lock_never_clears(monkeypatch):
    ...
    # The 503 body is the structured lock-contention contract, not prose: the
    # caller decides whether to retry from `retry_after_ms`, and a human-readable
    # sentence would have to be parsed to get there. Asserting the shape keeps
    # this test honest about what a client can actually act on.
    detail = r.json()["detail"]
    assert detail["reason"] == "lock_contention", detail
    assert detail["retry_after_ms"] > 0, detail
