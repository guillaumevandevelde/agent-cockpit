...
re-fetch `origin/master` and retry the merge once.

A conflict that is **exclusively** in the generated documentation artifacts
`docs/cockpit/README.md` and `docs/cockpit/llms.txt` is deterministic. **Verify
the conflict set before touching anything** — a handwritten conflict MUST go to
a human, never to this carve-out:

```bash
# Enumerate the unmerged paths and verify the set is EXACTLY the two
# generated doc-index files. Anything else (a hand-written conflict) aborts
# the carve-out and falls through to report_impediment below. The
# condition must be machine-checkable, not a judgement call — see
# kanban card efb8187b… for the failure mode that motivated this.
...
if [ "$CONFLICTED" != "$EXPECTED" ]; then
  echo "ERROR: conflict set is not exactly the generated doc-index pair — falling back to report_impediment." >&2
  printf '  conflicted: %s\n' $CONFLICTED >&2
  exit 1
fi
```

If the check passes, the conflict is deterministic. Resolve each generated file
with `--theirs` (the branch being merged just regenerated these from its own
frontmatter on its own ship; `generate-doc-index.py` overwrites the placeholder
on the next step anyway), then **regenerate from the surviving
`docs/cockpit/*.md` frontmatter inside `$WT`** — `scripts/generate-doc-index.py:78`
derives its repo-root from `Path(__file__).resolve().parent.parent`, so the
script MUST be invoked through the worktree path. A bare `cd $WT` is NOT enough:
the script resolves its parent relative to its own location, not to the shell's
cwd, so `./scripts/generate-doc-index.py` regenerates the wrong tree, the
strict check passes against the wrong tree, and the conflicted worktree ships
unmerged markers. Then complete the merge, push, and clean up:

```bash
git -C "$WT" checkout --theirs -- docs/cockpit/README.md docs/cockpit/llms.txt
"$WT"/scripts/generate-doc-index.py
"$WT"/scripts/generate-doc-index.py --check --strict
git -C "$WT" add -A
...
The strict check must pass before pushing. **`.gitattributes`-alternative
...
frontmatter added on the branch until someone manually re-runs the script. The
carve-out's "conflict → regenerate" loop is the right pattern: the conflict
acts as a freshness alarm, and the regenerate step reconciles from the merged
frontmatter.

If **any handwritten file is also in the conflict set**, the carve-out does
not apply: follow the existing rule, `report_impediment` naming all conflicting
files so a human can resolve it; never force-push or discard either side of the
conflict.
